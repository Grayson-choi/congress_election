def get_kor_amount_string(num_amount, ndigits_round=0, str_suffix='원'):
    ...
    if num_amount[0] == "-":
        ...
        if len(str_result) == 0:
            return None
        if not str_result[1].isnumeric():  # 앞이 숫자가 아닌 문자인 경우
            str_result = "-" + str_result  # 1을 붙인다
...
print(get_kor_amount_string("123,45"))
print(get_kor_amount_string("-784,354,243"))
print(get_kor_amount_string("123,456,789"))

def get_kor_amount_string_no_change(num_amount, ndigits_keep=3):
    """잔돈은 자르고 숫자를 자릿수 한글단위와 함께 리턴한다 """
    if num_amount[0] == "-":
        return get_kor_amount_string(num_amount, -(len(str(num_amount)) - ndigits_keep - 1))
    else:
        return get_kor_amount_string(num_amount, -(len(str(num_amount)) - ndigits_keep))

print(get_kor_amount_string_no_change("12,345"))
print(get_kor_amount_string_no_change("-123,456,789,230"))
